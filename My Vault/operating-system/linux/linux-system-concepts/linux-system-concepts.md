---
tags:
- os
- linux
- concept
title: Linux System Concepts
---

### Table of Content

* [File System Structure](file-system-structure.md)
* [Control Groups (cgroups)](control-groups-cgroups.md)
* [Linux Namespaces](linux-namespaces.md)
* [Device Drivers](device-drivers.md)
* [File Descriptors](file-descriptors.md)
* [Linux Kernel](linux-kernel.md)
* [Linux Memory Management](linux-memory-management.md)
* [Linux Environment Variables](../shell-scripting/linux-environment-variables.md)