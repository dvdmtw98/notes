...
* If during multiple inheritance there are two [classes](python-classes.md) that have the same [method](python-methods.md) then from method from the class that is inherited first by the child will only be available
* Private attributes from the parent class cannot be accessed in the child class
* When an method in the parent class is redefined in the child class it is called as [Method Overriding](python-method-overriding.md)
...
The [constructor](python-constructor.md)/ methods of the parent class can be accessed using the `super()` keyword
