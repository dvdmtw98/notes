...
Affects input fields and URLs that store there values as query parameters.  
If the input fields are not sanitized then we can enter a JS script which will get executed when submitted.  
Reflected XSS affects a single user and its results are instantly visible.
...
Effects all users that visit that page with the Stored XSS.  
